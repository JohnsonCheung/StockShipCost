...
Dim R1, R2, C1, C2, NC, NR
    ...
    NC = C2 - C1 + 1
    NR = R2 - C1 + 1
Ws_RCRC(PtWs(A), R1, C1, R2, C2).Copy
At.PasteSpecial xlPasteValues

Set PtCpyToLo = Rg_Lo(RgRCRC(At, 1, 1, NR, NC))
End Function
...
Dim F
For Each F In AyNz(Ssl_Sy(Rowss))
