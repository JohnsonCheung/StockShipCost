...
Const CSub$ = CMod & "Brk"
Dim P&: P = InStr(A, Sep)
If P = 0 Then XThw CSub, "{S} does not contains {Sep}", "S Sep", A, Sep
...
Sub Brk1Asg(A, Sep$, Optional O1, Optional O2, Optional NoTrim As Boolean)
...
Const CSub$ = CMod & "BrkAtAsg"
If At = 0 Then
    XThw CSub, "String does not have Sep", "Str Sep At NoTrim", A, Sep, At, NoTrim
    Exit Sub
End If
O1 = Left(A, At - 1)
O2 = Mid(A, At + Len(Sep))
