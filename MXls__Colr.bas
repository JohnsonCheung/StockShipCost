...
CvColr = ColrStr_Colr(A)
End Function
Function Colr_ColrStr$(A&)
Dim L
For Each L In ColrLy
    With Brk(L, " ")
        If .S2 = A Then Colr_ColrStr = .S1: Exit Function
    End With
Next
End Function
...
Sq = ColrSq
Set Ws = Rg_Ws(Sq_XPut_AtCell(ColrSq, New_A1))
For J = 1 To UBound(Sq, 1)
    Ws_RC(Ws, J, 3).Interior.Color = Sq(J, 2)
Next
Ws_CC(Ws, 1, 2).EntireColumn.AutoFit
Set ColrWb = Ws_Wb(Ws_XVis(Ws))
End Property
