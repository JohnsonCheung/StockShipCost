...
HasSqBkt = XTak_FstChr(A) = "[" And XTak_LasChr(A) = "]"
End Function

Function XHas_Chr_InLis(A, ChrList$) As Boolean
Dim J%
For J = 1 To Len(ChrList)
    If HasSubStr(A, Mid(ChrList, J, 1)) Then XHas_Chr_InLis = True: Exit Function
Next
End Function
